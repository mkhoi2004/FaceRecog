...
using FaceRecog.Dlib.Python;
using FaceRecog.Extensions;
...
namespace FaceRecog
{
    ...
    public sealed class FaceRecognition : DisposableObject
    {
        ...
        private FaceRecognition(string directory)
        {
            ...
            this._PosePredictor68Point = predictor68PointModel == null ? null : ShapePredictor.Deserialize(predictor68PointModel);
            ...
            this._CnnFaceDetector = cnnFaceDetectionModel == null ? null : LossMmod.Deserialize(cnnFaceDetectionModel);
        ...
        private FaceRecognition(ModelParameter parameter)
        {
            ...
            if (parameter.PosePredictor68FaceLandmarksModel == null)
                this._PosePredictor68Point = null;
            else
                                                       ...
                                                       Model model = Model.Hog)
        {
            ...
            if (predictorModel == PredictorModel.Custom)
                throw new NotSupportedException("FaceRecog.PredictorModel.Custom is not supported.");
        ...
        public static FaceEncoding LoadFaceEncoding(double[] encoding)
        {
            ...
            var matrix = new Matrix<double>(new MatrixTemplateSizeParameter(0, 1));
            matrix.SetSize(128);
                ...
                switch (predictorModel)
                {
                    ...
                    case PredictorModel.Large:
                        if (posePredictor == null)
                            throw new NotSupportedException("The 68-point face landmark model is not available. Please add shape_predictor_68_face_landmarks.dat.");
                        break;
                }

                if (posePredictor == null)
                    throw new NotSupportedException("The selected face landmark model is not available.");
        ...
        private IEnumerable<MModRect> RawFaceLocations(Image faceImage, int numberOfTimesToUpsample = 1, Model model = Model.Hog)
        {
            switch (model)
            {
                ...
                case Model.Cnn:
                    if (this._CnnFaceDetector == null)
                        throw new NotSupportedException("The CNN face detector model is not available. Please add mmod_human_face_detector.dat or use Hog.");
