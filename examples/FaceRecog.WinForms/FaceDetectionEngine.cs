...
namespace FaceRecog.WinForms
{
    internal sealed class FaceDetectionEngine
    {
        ...
        {
            using (var faceRecognition = FaceRecognition.Create(ResolveModelsDirectory(modelsDirectory)))
            {
        ...
        private string TestImage(FaceRecognition faceRecognition, string imageToCheck, Model model)
        {
            using (var unknownImage = FaceRecognition.LoadImageFile(imageToCheck))
            {
                ...
                if (faceLocations.Length == 0)
                    faceLocations = faceRecognition.FaceLocations(unknownImage, 1, model).ToArray();
        ...
        private static string ResolveModelsDirectory(string modelsDirectory)
        {
            ...
            return Data.ModelsDirectoryResolver.Resolve();
        }
    }
}