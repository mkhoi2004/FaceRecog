...
Đây là một ứng dụng WinForms riêng biệt dùng cho thư viện FaceRecog.
...
- `ModelsDirectory`: đường dẫn tới thư mục chứa các file model `.dat` của dlib. Mặc định là `models` cạnh file `.exe`, và ứng dụng sẽ tự dò thêm trong các thư mục cha nếu chưa tìm thấy.
...
Ứng dụng cần ít nhất 2 file model từ repo dlib:

- `shape_predictor_5_face_landmarks.dat`
- `dlib_face_recognition_resnet_model_v1.dat`

Bạn có thể đặt chúng trong thư mục `models` cạnh file thực thi, hoặc để `ModelsDirectory` mặc định là `models` như trong [App.config](App.config).

## Biên dịch

1. Mở solution.
2. Biên dịch `examples/FaceRecog.WinForms`.
